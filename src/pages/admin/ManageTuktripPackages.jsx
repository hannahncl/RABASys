...
import {
  Plus, Edit, Trash2, Loader, MapPin, Clock, Users, Navigation
} from 'lucide-react';
...
const ManageTuktripPackages = () => {
  ...
  const loadServices = async () => {
    setLoading(true);
    try {
      const data = await serviceService.getByCategory('tuktrip');
      setServices(data);
    } catch {
      showNotification('Failed to load tuktrips', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (item) => {
    navigate(`/admin/tuktrip/edit/${item.id}`);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this tuktrip?')) {
      try {
        await serviceService.delete(id);
        showNotification('Tuktrip deleted', 'success');
        loadServices();
      } catch {
        showNotification('Failed to delete tuktrip', 'error');
      }
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-display text-slate-100">Tuktrip</h1>
          <p className="text-slate-400 text-sm mt-0.5">
            {catConfig.description}
          </p>
        </div>
        <button
          onClick={() => navigate('/admin/tuktrip/add')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {services.length === 0 && (
            <div className="col-span-3 flex flex-col items-center justify-center h-40 text-center space-y-2">
              ...
              <p className="text-slate-500 text-sm">No tuktrips yet.</p>
              <button
                onClick={() => navigate('/admin/tuktrip/add')}
...
export default ManageTuktripPackages;
